...
It used to happen inline, inside POST /feeds, and it used to be followed by
...
sync itself. The feed row is what Library renders a card from, and the card
...
def run_initial_sync(feed_id: int, db: Session) -> None:
...
def run_initial_sync_task(feed_id: int) -> None:
